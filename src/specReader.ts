...
import { getOpenApiVersionFromSpec, OpenApiVersion } from './specVersion';
...
function getOpenApiVersionFromFiles(files: string[]) {
  const specs = files.map(path => JSON.parse(fs.readFileSync(path, 'utf8')));
